# -*- coding: utf-8 -*-
"""
Created on Sun Apr  9 04:04:03 2023
...
# Create the List for the other Actors
flower_list = []
wilted_list = []
fangflower_list = []

# Track the fangflowers
fangflower_vy_list = [] # This will hold the velocities of the fangflowers along the y-axis.
fangflower_vx_list = [] # This will hold the velocities of the fangflowers along the x-axis.
...
def draw():
    ...
    if not game_over:
        ...
        if (not garden_happy):
            ...
            finalized = True
        else:  # block of code runs if the garden is still happy but the cow has been zapped
            # draws a message on the screen to show the game is over because of a fangflower attack.
            screen.draw.text(
    ...
    if wilted_list: # This checks if there are any items in the wilted_list.
        for wilted_since in wilted_list: # code loops over each item in the wilted_list.
            if (not wilted_since == "happy"): # check if the flower is wilted and work out how long it’s been wilted.
                ...
                if (time_wilted) > 15.0: # Checks if the flower has been wilted for more than 15 seconds
                # Changes from 10 to 15 seconds for demenstration purposes
                    garden_happy = False
                    game_over = True
                    break
    return
...
def wilt_flower():
    ...
    if not game_over:
        ...
        if (flower_list[rand_flower].image == "flower"): # Checks if the flowers in the list are wilted or not
            flower_list[rand_flower].image = "flower-wilt" # Sets flower to wilted flower image
            wilted_list[rand_flower] = time.time() # Resets the time for this flower in wilted_list to current time
        clock.schedule(wilt_flower, 3) # Calls wilted_flower() every 3 seconds
    return

def check_flower_collision():
    global cow, flower_list, wilted_list # Global Variables
    index = 0 # index to move through the list
...
# Call the function to add more flowers
add_flowers()
# Call the function to start wilting
wilt_flower()
...
def update():
    ...
    if not game_over:
        if keyboard.space: # When Space bar is pressed
            cow.image = "cow-water" # Change cow image to one that is watering
            clock.schedule(reset_cow, 0.5) # Will Reset the cow's image after half a second
            check_flower_collision() # Will Check if the cow is next to the flower 
        if keyboard.left and cow.x > 0:
            ...
            cow.y += 5
        # checks if the garden has been happy for more than 15 seconds and if any fangflowers have appeared on the screen yet
        if time_elapsed > 15 and not fangflower_list:
            mutate() # Calls function to mutate
        update_fangflowers() 
